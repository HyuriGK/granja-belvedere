...
function renderPipeline(){
  const opportunities=db.oportunidades.filter(o=>o && o.titulo && o.etapa);
  document.getElementById("pipeline").innerHTML=stageConfig.map(stage=>{
    const items=opportunities.filter(o=>o.etapa===stage.id),sum=items.reduce((a,o)=>a+Number(o.valor),0);
    const summary=items.length?`${items.length} · ${money(sum)}`:"";
    return `<div class="pipeline-column" data-stage="${stage.id}"><div class="pipeline-head"><h3><i class="fa-solid fa-circle" style="color:${stage.color};font-size:7px"></i> ${stage.label}</h3><span>${summary}</span></div>${items.map(o=>`<div class="opportunity" draggable="true" data-id="${o.id}"><h4>${escapeHtml(o.titulo)}</h4><p>${escapeHtml(clientName(o.clienteId))}</p><div class="opportunity-foot"><strong>${money(o.valor)}</strong><span>${shortDate(o.previsao)}</span></div></div>`).join("")}</div>`
  }).join("");
